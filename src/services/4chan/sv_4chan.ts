...
export class Chan {
    ...
    async getRandomWsgWebm(): Promise<randomWebm> {
        const thread = await axios.get("https://a.4cdn.org/wsg/threads.json").then(r => {
            const pages = r.data;
            const randomPage = pages[getRandomInt(0, r.data.length - 1)]
            return randomPage.threads[getRandomInt(0, randomPage.threads.length - 1)] as threadData;
        })
        return await axios.get(`https://a.4cdn.org/wsg/thread/${thread.no}.json`).then(r => {
            const data = r.data
